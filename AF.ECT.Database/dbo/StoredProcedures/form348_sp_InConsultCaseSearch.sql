...
-- Description:	Returns all special cases in consult by WorkStatusId

-- ================================================================== 

CREATE PROCEDURE [dbo].[form348_sp_InConsultCaseSearch] 
@wsId INT
AS 
...
ELSE '' 
  ...
  JOIN Form348_SC f on f.status = w.ws_id 
...
) t								ON t.refId = f.SC_Id AND t.ws_id = f.status 

   LEFT JOIN core_lkupPAS pas		ON pas.pas = cs.PAS_CODE 

--left join (
--	select distinct(referenceId), actionDate, notes from core_LogAction --where notes like '%Digital Signature Generated%'
...
--) pcdI on pcdI.referenceId =	f.lodId and pcdI.notes = 'Complete'
 ...
 where f.workflow = @wsId and w.isConsult = 1

 --order by ws_id

--[dbo].[form348_sp_InConsultCaseSearch] 30, 6
GO
