-- Exec OverrideCase_SetToNewStatusRR '20120302-001-RR', 36
...
-- ============================================================================
CREATE Procedure [dbo].[OverrideCase_SetToNewStatusRR]
(
...
Begin
	...
	Begin
		...
		Update Form348_RR
		Set status = @newStatus, Cancel_Reason = 8, Cancel_Explanation = 'Case Cancelled by System Admin', Cancel_Date = GETDATE()
		, modified_by = @userId
		Where case_id = @CaseNumber
	...
	Begin
		If @isCancelStatus = 1
			Begin
				...
				Update Form348_RR
				Set status = @newStatus, Cancel_Explanation = Null, Cancel_Reason = Null, Cancel_Date = Null
				, modified_by = @userId
				Where case_id = @CaseNumber
			End
		Else
