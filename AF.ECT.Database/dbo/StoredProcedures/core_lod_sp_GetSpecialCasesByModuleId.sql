-- Exec core_lod_sp_GetSpecialCasesByModuleId 318, 13
-- ============================================================================
...
-- Description:		Added the selection of the 
--					ph_reporting_period field to the SELECT
--					query.
...
-- Description:		Ordered the dataset by ascending order
...
-- Description:		Returns the workflow title as well
-- ============================================================================
-- Modified By:		Austin Lorans
-- Modified Date:	11/4/2019
-- Description:		added vu.compo to SELECT and WHERE
-- ============================================================================
-- Modified By:		Curt Lucas
-- Modified Date:	11/9/2019
-- Description:		Added inner join on 'core_lkupPAS as pas' for pas priority sorting
-- ============================================================================
-- Modified By:		Curt Lucas
...
-- Description:		1. Removed unnecessary WHERE clause "AND vu.compo = @Compo"
--                  2. Fixed pas_priority formula modification from 11/15/2019
--					3. Syntax cleanup
-- ============================================================================
-- Modified By:		Curt Lucas
...
CREATE PROCEDURE [dbo].[core_lod_sp_GetSpecialCasesByModuleId]
	@userId INT,
	@moduleId INT
AS
...
-- Declare @userId int
-- SET @userId = 56   --295  -- 469 is system admin, 295 is MPF  
...
DECLARE @rptView int
DECLARE @userSSN CHAR(9)
SELECT 
	@rptView = ViewType, 
	@userSSN = SSN 
FROM vw_users WHERE userID = @userId
...
SELECT	DISTINCT sc_id
		...
		, DATENAME(MONTH, ph_reporting_period) + ' ' + DATENAME(YEAR, ph_reporting_period) AS Reporting_Period
...
WHERE	(cur.userRoleID = @roleId)
		...
		--AND (vu.compo = @Compo)											-- removed unnecessary clause
