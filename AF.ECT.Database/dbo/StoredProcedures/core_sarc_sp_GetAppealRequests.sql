...
-- Description:	Returns the set of APSA cases which are in a step that is 
...
-- ============================================================================
CREATE PROCEDURE [dbo].[core_sarc_sp_GetAppealRequests]
	@userId int,
	@sarc bit 
AS
BEGIN
	...
	SELECT @moduleId = moduleId FROM core_lkupModule WHERE moduleName = 'Restricted SARC Appeal'
	...
	FROM	Form348_AP_SARC ap
			Join Form348 lod ON lod.lodId = ap.initial_id AND lod.workflow = ap.initial_workflow
			...
			) t ON t.refId = ap.appeal_sarc_id AND t.ws_id = ap.status
	WHERE 
		ap.initial_workflow = lod.workflow
		AND lodVWS.isFinal = 1
		AND lodVWS.isCancel = 0  -- Initial LOD is Complete
		...
		(
			@scope > 1
			OR
			ap.member_ssn <> ISNULL(@userSSN, '---')	-- Don't return cases which revolve around the user doing the search...
		)
	UNION
	SELECT	DISTINCT appeal_sarc_id
			, SubString(ap.member_ssn, 6, 4) AS Protected_SSN
			...
			, ap.status
	FROM	Form348_AP_SARC ap
			Inner Join Form348_SARC sarc ON sarc.sarc_id = ap.initial_id AND sarc.workflow = ap.initial_workflow
			Inner Join vw_WorkStatus vws ON ap.status = vws.ws_id
			INNER JOIN vw_WorkStatus sarcVWS ON sarc.status = sarcVWS.ws_id
			Inner Join core_UserRoles cur ON cur.groupId = vws.groupId
			LEFT JOIN (
				SELECT Max(startDate) ReceiveDate, ws_id, refId 
				FROM core_WorkStatus_Tracking 
	...
	WHERE
		ap.initial_workflow = sarc.workflow
		AND
		(sarcVWS.isFinal = 1 AND sarcVWS.isCancel = 0)
		AND 
		(
			(
				@unitView = 1
