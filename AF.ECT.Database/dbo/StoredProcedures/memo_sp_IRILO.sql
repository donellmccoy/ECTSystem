...
-- Description:		Updated selected parameters.
-- ============================================================================
CREATE PROCEDURE [dbo].[memo_sp_IRILO]
	@refId int
AS
...
@gender2 varchar(6), 
@gender1a varchar(1), 
@expirationDate datetime,			-- Data Location: Expiration/Renewal data field in MB Board Med tab. 
@returnToDuty datetime,

-- NEW VARIABLES --
@diagnosisText varchar(200),		-- Data Location: diagnosis text box (row B) in WD Board Med tab. 
@dqParagraphInfo varchar(200),		-- Data Location: DQ Paragraph field in WD Board Med tab. 
...
SELECT 
	...
	,@orderName = MD.FIRST_NAME + ' ' + (CASE MD.MIDDLE_NAMES WHEN NULL THEN '' WHEN '' THEN '' ELSE LEFT(MD.MIDDLE_NAMES, 1) + ' ' END) +  MD.LAST_NAME
	, @unit = Member_Unit
	, @rank = Grade.RANK
...
WHERE SpecialCase.SC_Id = @refId
	...
	BEGIN
		...
		--	, @appAuthName = rr.sig_name_approval
			...
			, @returnToDuty = Alternate_Return_To_Duty_Date
			, @diagnosisText = FT_Diagnosis
			, @dqParagraphInfo = Alternate_DQ_Paragraph
...
INSERT INTO @results (id, value) VALUES ('MEMBER_FIRST', @orderName);
...
INSERT INTO @results (id, value) VALUES ('FOUR_SSN', @SSN);
