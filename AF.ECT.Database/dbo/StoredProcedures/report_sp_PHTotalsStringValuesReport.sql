...
-- Description:	Executes the PH Totals canned report to get the string valued
--				fields of the cases which meet the report parameters.
-- ============================================================================
...
-- ============================================================================
CREATE PROCEDURE [dbo].[report_sp_PHTotalsStringValuesReport]
	@unitId INT,
	@includeSubUnits BIT,
	@collocated TINYINT,
	@rptView INT,
...
BEGIN
	...
	SELECT	fs.[PH Wing RMU], 
			fs.[Reporting Period] AS RawReportingPeriod,
			DATENAME(MONTH, fs.[Reporting Period]) + ' ' + DATENAME(YEAR, fs.[Reporting Period]) AS [Reporting Period],
			fv.SectionId,
			fv.FieldId,
			fv.FieldTypeId,
	...
	FROM	PH_Form_Value fv
			JOIN PH_Form_Field ff ON fv.SectionId = ff.SectionId AND fv.FieldId = ff.FieldId AND fv.FieldTypeId = ff.FieldTypeId
			...
			JOIN vw_sc_ph_reporting fs ON r.RefId = fs.SC_Id
	WHERE	dt.Name LIKE 'String%'
			AND s.Name <> 'Comments'
	ORDER	BY fs.[PH Wing RMU], fs.[Reporting Period], s.Name, f.Name
END
GO
