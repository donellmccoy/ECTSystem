...
2. **Consolidate LOD folders**: Merge `LineOfDuty/` and `LOD/` or document separation
   - If keeping both, create README explaining the distinction
   - Ensure routing is clear and non-conflicting

3. **Create role constants file**: Centralize authorization role strings
...
2. **Documentation**: Add folder-level README files explaining module organization
   - Document each module's purpose
   - Explain routing patterns and authorization requirements

3. **Component library**: Build shared UI components for common patterns
